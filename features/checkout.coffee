	class Commerce.Checkout
		...
		protect: (token) ->
			@c.Request 'checkout/'+token+'/protect',
								 'GET',
								 null,
								 (data) ->
									 eval data.sift_js;
	  ...
	  checkPaypalStatus: (token, callback, error) ->
	    @c.Request 'checkout/' + token + '/check/paypal/payment', 'GET', {}, callback, error

	  checkPaypalOrderCaptured: (token, callback, error) ->
	    @c.Request 'checkout/' + token + '/check/paypal/captured', 'GET', {}, callback, error

	  receipt: (token, callback, error) ->
	    @c.Request 'checkout/' + token + '/receipt', 'GET', {}, callback, error

	  checkPayWhatYouWant: (token, customer_set_price, callback, error) ->
	    @c.Request 'checkout/' + token + '/check/pay_what_you_want', 'GET', { 'customer_set_price': customer_set_price }, callback, error

	  fields: (identifier, callback, error) ->
	    @c.Request 'checkout/' + identifier + '/fields', 'GET', null, callback, error
	  ...
	  setTaxZone: (identifier, location, callback, error) ->
	    ...
	    @c.Request 'checkout/' + identifier + '/helper/set_tax_zone', 'GET', {
	  ...
	  getLocationFromIP: (token, ip_address, callback, error) ->
	    ...
	    if typeof ip_address == 'function'
	      @c.Request 'checkout/' + token + '/helper/location_from_ip', 'GET', null, ip_address, error
	    else
	      @c.Request 'checkout/' + token + '/helper/location_from_ip', 'GET', { 'ip_address': ip_address }, callback, error

	  isFree: (token, callback, error) ->
	    @c.Request 'checkout/' + token + '/check/is_free', 'GET', null, callback, error

	  checkVariant: (token, line_item_id, variant_id, option_id, callback, error) ->
	    @c.Request 'checkout/' + token + '/check/' + line_item_id + '/variant', 'GET', {
	      'variant_id': variant_id
	      'option_id': option_id
	    }, callback, error

	  checkDiscount: (token, code, callback, error) ->
	    @c.Request 'checkout/' + token + '/check/discount', 'GET', { 'code': code }, callback, error

	  checkShippingOption: (token, shipping_country, id, callback, error) ->
	    @c.Request 'checkout/' + token + '/check/shipping', 'GET', {
	  ...
	  helperValidation: (token, callback, error) ->
	    @c.Request 'checkout/' + token + '/helper/validation', 'GET', null, callback, error

	  getLive: (token, callback, error) ->
	    @c.Request 'checkout/' + token + '/live', 'GET', null, callback, error
